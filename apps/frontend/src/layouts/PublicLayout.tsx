...
import { ChefHat, FileWarning, MapPin, Phone } from 'lucide-react';
...
export function PublicLayout() {
  ...
  return (
    // `MotionConfig reducedMotion="user"` hace que TODA animación de framer-motion bajo la
    // carta pública respete `prefers-reduced-motion` sola, sin repetir la condición en cada
    // componente (mismo criterio que ya aplicaba `useMovimientoReducido` a mano).
    <MotionConfig reducedMotion="user">
      {/* `data-tema="carta"` activa la paleta propia de la superficie pública (ver
          index.css), que sigue el modo claro/oscuro del sistema. El panel interno no la usa. */}
      <div
        data-tema="carta"
        ...
        <footer className="border-t border-(--carta-borde)">
          <div className="mx-auto flex max-w-6xl flex-col gap-3 px-5 py-10 text-sm text-(--carta-suave) sm:flex-row sm:items-center sm:justify-between sm:px-8">
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              ...
              {/* Debe ser visible y accesible sin autenticarse — es lo que exige el
                  reglamento del Libro de Reclamaciones (Ley 29571). */}
              {slug && (
                <Link
                  to={`/libro-reclamaciones/${slug}`}
                  className="flex items-center gap-2 transition-colors hover:text-(--carta-acento)"
                >
