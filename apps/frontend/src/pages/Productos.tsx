        ...
        <div className="mt-3 overflow-hidden rounded-lg border border-zinc-200 bg-white">
          <ul className="divide-y divide-zinc-100">
            {lineas.map((linea, indice) => {
              ...
              const costoLinea =
                insumo?.ultimoCosto != null ? insumo.ultimoCosto * linea.cantidad : null;
              return (
                <li key={linea.insumoId} className="flex items-center gap-2 px-3 py-2 text-sm">
                  ...
                  <span className="w-16 shrink-0 text-right text-xs text-zinc-400 tabular-nums">
                    {costoLinea != null ? formatearPrecio(costoLinea) : '—'}
                  </span>
                  <button
                    type="button"
          ...
          <div className="flex items-center justify-between border-t border-zinc-200 bg-zinc-50 px-3 py-2 text-xs">
            <span className="text-zinc-500">
              Costo estimado por unidad
              {lineas.some((l) => insumoPorId.get(l.insumoId)?.ultimoCosto == null) &&
                ' (incompleto: falta el costo de compra de algún insumo)'}
            </span>
            <span className="font-semibold text-zinc-900 tabular-nums">
              {formatearPrecio(
                lineas.reduce((suma, l) => {
                  const costo = insumoPorId.get(l.insumoId)?.ultimoCosto;
                  return suma + (costo != null ? costo * l.cantidad : 0);
                }, 0),
              )}
            </span>
          </div>
