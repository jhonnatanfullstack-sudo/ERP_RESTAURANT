...
| `talonarios.ver` / `.crear` / `.editar` / `.eliminar` / `.asignar` | Series de comprobantes y su correlativo. `.asignar` es propio del módulo: decidir qué cajero emite desde qué serie es una decisión distinta de editar el talonario. **Emitir** desde un talonario no usa ninguno de estos — se resuelve con `ventas.crear` (endpoint `GET /api/talonarios/mios`) |
| `cobranzas.ver` / `.registrar` / `.anular`                         | Documentos por cobrar de las ventas al crédito. No usa `.crear/.editar/.eliminar`: un cobro se registra y, si se equivocaron, se anula dejando el motivo — mismo criterio que Ventas y Compras                                                                                                   |
...
| `proveedores.ver` / `.crear` / `.editar` / `.eliminar`             | Gestión de proveedores (`.eliminar` = desactivar: un proveedor ya puede tener compras registradas)                                                                                                                                                                                               |
| `compras.ver` / `.crear` / `.editar` / `.anular`                   | Compras a proveedores. `.anular` en vez de `.eliminar`: una compra ya movió stock, se reversa con movimientos `anulacion_compra`, nunca se borra                                                                                                                                                 |
| `reportes.ver`                                                     | Reportes y analítica del negocio (FASE 19). Un solo permiso de lectura, pero expone el total vendido y el ranking de clientes: no se abre a cualquier rol                                                                                                                                        |
| `costos.ver`                                                       | Costo y margen de cada producto (FASE 18). Lectura derivada de recetas y compras ya registradas. Es distinto de `productos.editar` (que gobierna la receta en sí): quién define qué lleva un platillo no es necesariamente quién puede ver cuánto gana el negocio con él                         |
| `auditoria.ver`                                                    | Bitácora de auditoría del sistema (FASE 20)                                                                                                                                                                                                                                                      |
| `configuracion.ver` / `.editar`                                    | Parámetros operativos del restaurante (FASE 21): duración de reserva, refresco de cocina, días de crédito, umbrales de food cost y los datos públicos de la carta. Sin `.eliminar`: la configuración no se borra, se deja en sus valores por defecto                                             |
...
## Multi-empresa: el RBAC es interno de cada restaurante (FASES 25-26, 2026-09-10)
