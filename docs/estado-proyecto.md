...
- **Editar/eliminar + validaciones (2026-09-07)**: Usuarios/Personal/Roles ahora tienen editar y eliminar en el frontend (antes solo Empresa). `personal.eliminar`/`usuarios.eliminar` = desactivar (con cascada personal→usuario); `roles.eliminar` = borrado real (bloqueado si hay usuarios con ese rol). El número de documento de `Personal` valida su formato según el tipo (DNI=8 dígitos, RUC=11) en backend y frontend. Botón de búsqueda RENIEC/SUNAT vía **apis.net.pe** (decisión del usuario) — funciona solo si se configura `APIS_NET_PE_TOKEN` en `.env` (opcional, degrada con 503 si falta). Ver `api.md` y `roles-y-permisos.md`.
- **Rediseño visual (2026-09-07)**: tipografía "Plus Jakarta Sans", acento naranja sobre neutros `zinc`, iconos `lucide-react`, componentes `Button`/`Badge`/`StatCard`/`EmptyState`/`ConfirmDialog` reutilizables. Ver `frontend.md`.
